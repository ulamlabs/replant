...
def test_listing_trees_clusters(user_client: APIClient):
    ...
    response = user_client.get("/api/trees_clusters")
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == ["zoom and index params must be provided"]

    response = user_client.get("/api/trees_clusters?index=1000&zoom=1")
    ...
    response = user_client.get("/api/trees_clusters?index=1000&zoom=2")
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert len(data) == 1
    assert data[0]["number_of_trees"] == 300

    response = user_client.get("/api/trees_clusters?index=1001&zoom=1")
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert len(data) == 1
    assert data[0]["number_of_trees"] == 400

    response = user_client.get("/api/trees_clusters?index=1002&zoom=3")
    assert response.status_code == status.HTTP_200_OK
    assert response.json() == []
